...
- **Visual separation on Today**: `renderToday()` in app.js splits the day's items into a "Habits" group and a "Tasks" group (`.task-group-label` headers), each independently sorted done-to-bottom. Tasks render two-tone peach + teal (`TASK_THEME`/`LIGHT_TASK_THEME` in habits.js — `accent` is peach, used for the card border/tint; `accent2` is teal, used only for the checkbox via `--tone-accent-2` in `applyTone()`) instead of the habit's own rotated color — intentional so the eye can bucket "temporary, needs clearing" apart from a habit's "ongoing, has its own identity" at a glance. Completed items (both groups) stay visible, dimmed + struck through, rather than disappearing — the user explicitly wants a visible record of what got done, not a shrinking counter.

...
- **Swipe left/right to change day** (`app.js:initDaySwipe`, bound to `#view-today`): mimics Apple Calendar's day-view paging — a purely horizontal drag of at least 60px anywhere on the Today screen moves `selectedDate` by ±1 day. It explicitly ignores drags starting inside `.swipe-wrapper` (a card's own edit/delete reveal), or on any `button`/`input`, so it never fights those gestures for the same touch.

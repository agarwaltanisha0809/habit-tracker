...
- **Carry-over** (`storage.js:getOverdueOnceTasks/carryOverTask/dismissCarryOver`, `app.js:checkCarryOver`, `#carryOverModal`): once per real day, **only for a task dated exactly yesterday** (`getOverdueOnceTasks` filters `schedule.date === addDays(state.date, -1)`, not "any date before today") — a modal lists it with a checkbox (checked by default), Continue moves checked ones to today and marks the rest dismissed for today, closing with X dismisses all. A task left undone for longer than a day just sits there quietly; it does **not** keep getting asked about on later days, and can still be brought forward manually via the "Tomorrow" swipe action. Also gated to not fire before 6am local time (`checkCarryOver` returns early if `new Date().getHours() < 6`) — the user works past midnight some nights and doesn't want "new day" prompts while they're still up. Applies to one-off tasks only, never recurring habits.

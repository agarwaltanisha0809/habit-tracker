...
- **"Move to tomorrow" swipe action** (one-off tasks only, not habits): a third swipe action on task cards, alongside Edit and Delete, that calls `carryOverTask(habit.id, addDays(date, 1))` — the same function the automatic next-day carry-over prompt uses, just applied immediately and manually instead of waiting. Lets you clear a task off today's list without losing it or having to retype it tomorrow.
